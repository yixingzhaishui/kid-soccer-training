...
export function personalizeScenario(scene:AnimatedScenario,childName:string):AnimatedScenario{
  ...
  return {
    ...
    actors:scene.actors.map((actor)=>actor.id==='nolan'?{...actor,name:displayChildName(childName)}:actor),
    choices:scene.choices.map((choice)=>({...choice,spokenLabel:text(choice.spokenLabel),label:text(choice.label)})),
    results:scene.results.map((result)=>({...result,narration:text(result.narration),explanation:text(result.explanation),teamEffect:text(result.teamEffect),comparisonLine:text(result.comparisonLine)})),
  };
}
