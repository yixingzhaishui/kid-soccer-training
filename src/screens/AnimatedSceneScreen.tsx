...
type Props={scene:AnimatedScenario;sceneIndex:number;total:number;score:number;sceneScore:number;onHome:()=>void;onBack:()=>void;onSpeak:(text:string)=>void;onResult:(result:ChoiceResult,awardScore:boolean)=>void;onNext:()=>void};
...
export function AnimatedSceneScreen({scene,sceneIndex,total,score,sceneScore,onHome,onBack,onSpeak,onResult,onNext}:Props){
  const[phase,setPhase]=useState<Phase>('watch');const[runKey,setRunKey]=useState(0);const[selected,setSelected]=useState<ChoiceResult|undefined>();const[scoreUsed,setScoreUsed]=useState(false);const[earned,setEarned]=useState<number|undefined>();const setupFrame=useMemo(()=>finalFrame(scene.actors,scene.ballStart,scene.setupAnimation),[scene]);
  const setupDone=useCallback(()=>{setPhase('choose');onSpeak(scene.stage===2?'What will happen next?':scene.prompt)},[onSpeak,scene]);
  useEffect(()=>{setPhase('watch');setSelected(undefined);setScoreUsed(false);setEarned(undefined);setRunKey((key)=>key+1);onSpeak(scene.introNarration)},[scene,onSpeak]);
  const pick=(choiceId:string)=>{const found=scene.results.find((item)=>item.choiceId===choiceId)!;const canScore=!scoreUsed;setSelected(found);setEarned(canScore?Math.max(0,pointsForResult(found)-sceneScore):undefined);setScoreUsed(true);setPhase('result');onResult(found,canScore)};
  ...
  return <main className="screen scene-screen"><header className="scene-header"><button className="icon-button" onClick={onBack} aria-label="Back to stories">‹</button><div><strong>{scene.title}</strong><span>{sceneIndex+1} of {total} · Stage {scene.stage}</span></div><div className="scene-score" aria-label={`${score} total points`}>⭐ {score}</div><button className="icon-button" onClick={onHome} aria-label="Go home">⌂</button></header>
    ...
    <SceneCanvas actors={scene.actors} ballStart={scene.ballStart} steps={steps} activeArea={scene.activeArea} initial={initial} runKey={runKey} onComplete={phase==='watch'||phase==='choose'?setupDone:phase==='result'||phase==='explain'?resultDone:undefined}/>
