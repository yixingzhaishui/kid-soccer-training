...
export type ActiveArea={x:number;y:number;width:number;height:number;label:string};
export type Team='blue'|'red';
export type SceneCategory='winger'|'striker'|'central-midfielder'|'attacking-midfielder'|'defensive-midfielder'|'fullback'|'center-defender'|'goalkeeper'|'teamwork';
...
export type AnimatedScenario={
  ...
  skillId:string;visibleCue:string;playerDuty:string;
  introNarration:string;prompt:string;actors:AnimatedActor[];ballStart:Point;setupAnimation:AnimationStep[];
  pauseTime:number;activeArea:ActiveArea;choices:AnimatedChoice[];results:ChoiceResult[];
};
